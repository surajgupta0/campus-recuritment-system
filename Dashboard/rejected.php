...
WHERE stu_id={$_SESSION['student_id']} AND status='{$rejected}' {$categoryall} LIMIT {$offset},{$limit}
...
if(mysqli_num_rows($result)>0){
    ...
    WHERE stu_id={$_SESSION['student_id']} AND status='{$rejected}' {$categoryall}";
   ...
   if(mysqli_num_rows($result5)>$limit){
    ...
    for($i=1;$i<=$total_page;$i++){
        ...
        echo"<td><a href='rejected.php?page=$i'class='$active'>$i</a></td>";
    }
    echo"</tr></table>";
   }
}
...
WHERE id={$_SESSION['company_id']} AND  status='{$rejected}' {$categoryall} ORDER BY main_id DESC  LIMIT {$offset},{$limit}
...
if(mysqli_num_rows($result)>0){
    ...
    WHERE id={$_SESSION['company_id']} AND  status='{$rejected}'  {$categoryall}";
   ...
   if(mysqli_num_rows($result5)>$limit){
    ...
    for($i=1;$i<=$total_page;$i++){
        ...
        echo"<td><a href='applied_student.php?page=$i'class='$active'>$i</a></td>";
    }
    echo"</tr></table>";
   }
}
