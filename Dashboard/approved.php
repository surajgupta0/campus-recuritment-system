...
WHERE stu_id={$_SESSION['student_id']} AND status='{$Approved}' {$categoryall}  LIMIT {$offset},{$limit}
...
if(mysqli_num_rows($result)>0){
    ...
    WHERE stu_id={$_SESSION['student_id']} AND status='{$Approved}'";
   ...
   if(mysqli_num_rows($result5)>$limit){
    ...
    for($i=1;$i<=$total_page;$i++){
        ...
        echo"<td><a href='approved.php?page=$i'class='$active'>$i</a></td>";
    }
    echo"</tr></table>";
   }
}
...
WHERE id={$_SESSION['company_id']} AND  status='{$approved}' {$categoryall} ORDER BY main_id DESC  LIMIT {$offset},{$limit}
...
if(mysqli_num_rows($result)>0){
    ...
    WHERE id={$_SESSION['company_id']} AND  status='{$approved}'  {$categoryall}";
